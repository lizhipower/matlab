function [MVAbase, bus, gen, branch, success, et] = ...
    			runpf(baseMVA, bus, gen, branch, mpopt, fname, solvedcase)
...
[ref, pv, pq] = bustypes(bus, gen);%要求调用者将数据中的bus和gen对应类型改变
...
if dc                               %% DC formulation
    ...
    V0(gbus) = gen(on, VG) ./ abs(V0(gbus)).* V0(gbus);%发电机设定电压的以发电机为准
    ...
    while (repeat)
        ...
        %% update data matrices with solution
        [bus, gen, branch] = pfsoln(baseMVA, bus, gen, branch, Ybus, Yf, Yt, V, ref, pv, pq);%更新传入的数据，其中支路潮流保存进了branch中，电压幅值保存在了bus的VM里面
